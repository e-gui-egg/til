...
export async function GET({ site }: { site: URL }) {
  ...
  const items = issues
    ...
    .join("");
  return new Response(
    `<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>Issue Journal</title><link>${site}</link><description>GitHub Issues から生成する技術ノート</description>${items}</channel></rss>`,
